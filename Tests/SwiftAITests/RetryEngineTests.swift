...
struct RetryEngineTests {
    ...
    func permanentErrorNoRetry() async throws {
        ...
        do {
            let _: String = try await engine.execute {
                await counter.increment()
                throw SwiftAIError.authenticationFailed(.anthropic)
            }
    ...
    func networkTimeoutRetryable() {
        ...
        #expect(engine.isRetryable(SwiftAIError.networkError(underlying: URLError(.timedOut))))
    }

    @Test("Rate limit is retryable")
    func rateLimitRetryable() {
        let engine = RetryEngine()
        #expect(engine.isRetryable(SwiftAIError.rateLimited(.openAI, retryAfter: .seconds(5))))
    }

    @Test("HTTP 429 is retryable")
    func http429Retryable() {
        let engine = RetryEngine()
        #expect(engine.isRetryable(SwiftAIError.httpError(statusCode: 429, body: "")))
    }

    @Test("HTTP 503 is retryable")
    func http503Retryable() {
        let engine = RetryEngine()
        #expect(engine.isRetryable(SwiftAIError.httpError(statusCode: 503, body: "")))
    }

    @Test("HTTP 400 is not retryable")
    func http400NotRetryable() {
        let engine = RetryEngine()
        #expect(!engine.isRetryable(SwiftAIError.httpError(statusCode: 400, body: "")))
    }

    @Test("Auth failure is not retryable")
    func authNotRetryable() {
        let engine = RetryEngine()
        #expect(!engine.isRetryable(SwiftAIError.authenticationFailed(.anthropic)))
    }

    @Test("Invalid request is not retryable")
    func invalidRequestNotRetryable() {
        let engine = RetryEngine()
        #expect(!engine.isRetryable(SwiftAIError.invalidRequest(reason: "bad")))
    }

    @Test("Backoff delay increases exponentially")
    func exponentialBackoff() {
        let engine = RetryEngine(baseDelay: .seconds(1), maxDelay: .seconds(60))
        let error = SwiftAIError.networkError(underlying: URLError(.timedOut))
