...
private let logger = Logger(subsystem: "com.arbiter", category: "UsageAnalytics")
...
public actor UsageAnalytics {
    ...
    private let persistenceKey = "com.arbiter.usage_analytics"

    public init() {
        let store = UserDefaults(suiteName: "com.arbiter.analytics") ?? .standard
        self.defaults = store
        if let data = store.data(forKey: "com.arbiter.usage_analytics") {
